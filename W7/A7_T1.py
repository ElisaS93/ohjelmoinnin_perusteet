...
def displayIntegers(Numbers: list[int]) -> None:
    if len(Numbers) == 0:
        ...
        for i in range(len(Numbers)):
            print(f"- Index {i} => Ordinal {i+1} => {Numbers[i]}")
    return None
...
def main() -> None:
    ...
    while CurrentInteger >= 0:
        PositiveIntegers.append(CurrentInteger) #lisätään listan viimeiseksi appendilla
        CurrentInteger = askPositiveInteger()
    print("Stopped collecting positive integers.")
    
    displayIntegers(PositiveIntegers)
    PositiveIntegers.clear() #tyhjentää listan
